...
설치하지 않아도 `./gradlew :sim:run` 한 번으로 브로커 + 발행자(publisher, 라인당 1개)가
전부 뜹니다.
...
generator/
 ...
 │       └─ mqtt/                    EmbeddedBroker(Moquette), RoomEnvPublisher
...
다시 쓰는 Kafka Streams 앱(`linestate-streams` 모듈)이 있다. 그 결과를 각각 독립된
